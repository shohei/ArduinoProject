...
//if you want to set for Saturday and Sunday also,
//set SCHEDULE_DAYS to 6 or 7
...
void setup() {
  ...
  setTime(7,59,50,24,9,12); // set time to Saturday 8:29:00am Jan 1 2011  
  ...
  for (int i = 0; i < SCHEDULE_DAYS; i++){
    for(int j =0; j < SCHEDULE_SIZE; j++){  
     Alarm.alarmRepeat(dayOfWeeks[i],schedule[j][0],schedule[j][1],0,Silen);  // Monday to Friday every week
    }
  }
